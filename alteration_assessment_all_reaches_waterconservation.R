#Alteration assessment for all stream reaches from LSPC model - Water Conservation Scenarios using updated recalibration LSPC 
...
fnames <- list.files(water.con.dir, pattern = "\\.out$")
...
for (i in 1:length(fnames)){
  ...
  names(water.con) <- c("gage", "year", "month", "day", "hour", "min", "precip", "depth", "av.depth", "av.vel","flow.cfs")
  #format date
