#Flow component alteration map - FFM maps, heat maps, synthesis WATER CONSERVATION MAPS
...
#load in data
alt.dir.name <- "WaterConservation_Aliso_SmCk_SanJuan_all_lowflowbias/" #update to directory name with data to be analyzed
...
ggsave(mine.heatmap, file="C:/Users/KristineT.SCCWRP2K/Documents/Git/SOC_FESS/heatmap_alteration_allsubbasins_waterconservation.jpg", dpi=300, height=8, width=12)
...
ggsave(mine.heatmap2, file="C:/Users/KristineT.SCCWRP2K/Documents/Git/SOC_FESS/heatmap_alteration.nofreqROC.allsubbasins_waterconservation.jpg", dpi=400, height=8, width=10)
...
data$scenario <- "Water Conservation"
#write.csv
filename.data <- paste0(alteration.dir, "ffm_alteration_maps_dataset_compile_waterconservation.csv")
...
comp_alt_synth$altered_components <- factor(comp_alt_synth$flow_component, levels = c("All", "Wet-season, Dry-season", "Wet-season, Peak Flow", "Dry-season, Peak Flow", "Dry-season", "Wet-season", "Peak Flow", "None"))
colors <- c("#a50f15", "#d95f0e", "#fdae61", "#fd8d3c", "#fff7bc", "#fee090", "pink", "#4575b4")
levels <- c("All", "Wet-season, Dry-season", "Wet-season, Peak Flow", "Dry-season, Peak Flow", "Dry-season", "Wet-season", "Peak Flow", "None")
...
#save image
plot.fname <- paste0(dir.alt, "Synthesis_Alteration_Map_wetdrypeak_allsubbasins_waterconservation.jpg")
...
data.current <- read.csv("L:/San Juan WQIP_KTQ/Data/RawData/From_Geosyntec/South_OC_Flow_Ecology_for_SCCWRP/KTQ_flowalteration_assessment/Aliso_Oso_SmCk_SanJuan_all_lowflowbias/ffm_alteration_maps_dataset_compile_current.csv")
...
data.watercon <- data %>% 
  ...
  rename(alteration.status.watercon = alteration.status, 
         ...
         alteration.status.new.watercon = alteration.status.new)
...
data.all <- data.all %>% 
  mutate(alteration.status.change = paste(alteration.status.new, "to", alteration.status.new.watercon, sep=" "),
...
data.all$alteration.status.change[data.all$alteration.status.new == data.all$alteration.status.new.watercon] <- "No Change"
unique(data.all$alteration.status.change)
...
unique(data.all$alteration.status.change.simple)
#change anything to NA as indeterminate
data.all$alteration.status.change.simple[data.all$alteration.status.change.simple == "Likely Altered to NA"] <- "Indeterminate"
data.all$alteration.status.change.simple[data.all$alteration.status.change.simple == "Likely Unaltered to NA"] <- "Indeterminate"
data.all$alteration.status.change.simple[data.all$alteration.status.change.simple == "Indeterminate to NA"] <- "Indeterminate"
#indeterminate/NA to likely Altered
data.all$alteration.status.change.simple[data.all$alteration.status.change.simple == "Indeterminate to Likely Altered"] <- "Indeterminate/NA to Likely Altered"
data.all$alteration.status.change.simple[data.all$alteration.status.change.simple == "NA to Likely Altered"] <- "Indeterminate/NA to Likely Altered"
#indeterminate/NA to likely Unaltered
data.all$alteration.status.change.simple[data.all$alteration.status.change.simple == "Indeterminate to Likely Unaltered"] <- "Indeterminate/NA to Likely Unaltered"
data.all$alteration.status.change.simple[data.all$alteration.status.change.simple == "NA to Likely Unaltered"] <- "Indeterminate/NA to Likely Unaltered"
...
alteration.status.change.simple <- c("Likely Unaltered to Likely Altered", "Indeterminate/NA to Likely Altered", "No Change", "Indeterminate/NA to Likely Unaltered", "Likely Altered to Likely Unaltered", "Indeterminate",  NA)
...
for(j in 1:length(unique.ffm)){
  ...
  #write plot
  #save as jpg
  plot.fname <- paste0(dir.alt,unique(basins.all.sub$ffm), "_alteration.change.map.jpg")
