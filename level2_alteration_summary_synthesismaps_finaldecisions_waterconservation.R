#### Level 2 - alteration summary based on various thresholds WATER CONSERVATION SCENARIO
...
#file name 
fname <- paste0(level2.dir, "09_metric_suitability_tally_condensed_all_sites_watercons_June2021.csv") #all lspc subbasins including San Juan and Oso/small tribs, Aliso and low flow bias corrected data
...
out.dir <- paste0(level2.dir, "Suitability_Maps/", "prob25CSCI.75ASCI_75time_waterconservation/")
dir.create(out.dir)
...
#model source LSPC or Wildermuth - do not need model source
...
####################################
#DECISIONS: Use the probability threshold and bio threshold combo that provides the most discriminatory power from Katie's analysis
#OLD: Use 50% probabilty threshold
#suit_data2_50 <- suit_data2[suit_data2$Threshold == "Threshold50",]
suit_data2_50 <- data.frame(suit_data2) #dont need to subset since it already contains the appropriate prob threshold, just use as is
...
#save csv
file.name <- paste0(level2.dir, "summary.50prob.75pct.time.altered.2metrics_LSPC_All_WaterConservation_06302021.csv")
...
#tally number of biol (csci/asci) altered per site
subset.75pct.time.summary2.all <- subset.75pct.time.all2 %>% 
...
#write plot
out.filename <- paste0(out.dir, "Synthesis_Prioritization_map_LSPC_all_Aliso_Oso_small_creeks_SJ_WaterConservation.jpg")
...
############################################################
#create suitability maps for appropriate prob and biol threshold combos, for 50% of time altered altered, 2 metrics

#names
#cols <- names(subset.75pct.time.all)
cols <- names(subset.75pct.time.all2)
...
for(i in 7){
  ...
  for(z in indices){
    ...
    for(q in unique.prob.thresholds){
      ...
      #write plot
      out.filename <- paste0(out.dir, z, "_alteration_map_LSPC_Aliso_Oso_small_creeks_SJ.jpg")
